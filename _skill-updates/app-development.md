...
- PWA manifest and service worker already configured — do not overwrite
- Guest mode: 7-day hard-countdown trial (no indefinite guest access — ever)
...
| 7-day hard-countdown trial | ✅ Built | Hard block at expiry — no soft gates |
...
- Accepts a base64 image of a pool shop water test printout
- Calls Claude Vision API to extract chemical readings
- Returns structured JSON: `{ pH, chlorine, alkalinity, cyanuricAcid, calcium, ... }`
- Rate limited: 10 calls/user/hour
- Auth required: Supabase JWT must be valid

When building any feature that touches water test data, check if OCR pre-fill is appropriate.
